...
class SerialServo:
    ...
    def receive_command(self, expected_cmd: int, expected_data_len: int) -> list:
        ...
        # 接收数据
        data = self.uart.read(expected_data_len+3)
    ...
    def move_servo_immediate(self, servo_id: int, angle: int, time_ms: int) -> None:
        """
        立即控制舵机转动到指定角度。
        ...
        Args:
            servo_id (int): 舵机ID，范围0~253。
            angle (int): 目标角度（0~240度范围内）。每个单位表示 0.24 度。实际控制值为0~1000
        ...
        if angle < 0 or angle > 1000:
            raise ValueError("Angle must be in range 0~1000.")
